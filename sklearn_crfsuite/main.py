...
X = [[features_default(x, i, 0.2) for i in range(len(x))] for x in tokens]
X = [pycrfsuite.ItemSequence(x) for x in X]

pred = crf.predict(X)[0]

tei_generator = TEIGen([x.string for x in tokens[0]], pred, ["doctor"], [x.page_num for x in tokens[0]], 5)
